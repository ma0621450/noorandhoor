...
function formSubmitSubject(payload) {
  const name = payload.name || payload.fullName || "website";
  if (payload.source === "newsletter") {
    return `New newsletter signup: ${payload.email}`;
  }
  if (payload.source === "contact-page") {
    return `New contact form enquiry from ${name}`;
  }
...
async function submitViaFormSubmit(payload) {
  ...
  const response = await fetch(
    `https://formsubmit.co/ajax/${encodeURIComponent(FORM_SUBMIT_TO)}`,
    {
      ...
      body: JSON.stringify({
        _subject: formSubmitSubject(payload),
        _template: "table",
        _captcha: "false",
        name,
        email: payload.email,
