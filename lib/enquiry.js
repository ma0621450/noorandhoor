...
export function getLeadToEmail() {
  return cleanLine(
    process.env.LEAD_TO_EMAIL ||
      process.env.NEXT_PUBLIC_LEAD_EMAIL ||
      "jamshaid13579@gmail.com",
    320,
  );
}

export function enquirySubject(data) {
  if (data.source === "newsletter") {
    return `New newsletter signup: ${data.email}`;
  }
  return `New enquiry from ${data.name}`;
}
  ...
  if (source === "newsletter") {
    return {
      ...
      data: {
        ...
        company: "",
        service: "Newsletter",
        details: "Requested newsletter updates.",
        brief: "Requested newsletter updates.",
