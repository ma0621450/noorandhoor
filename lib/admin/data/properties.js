...
function persist(next) {
  ...
  try {
    window.localStorage.setItem(ADMIN_PROPERTIES_KEY, raw);
  } catch {
...
export function getPropertiesSnapshot() {
  ...
  if (!raw) {
    snapshot = ADMIN_PROPERTIES.map(hydrateProperty);
    return snapshot;
  }

  try {
    const parsed = JSON.parse(raw);
    snapshot = Array.isArray(parsed) ? parsed.map(hydrateProperty) : ADMIN_PROPERTIES.map(hydrateProperty);
  } catch {
    snapshot = ADMIN_PROPERTIES.map(hydrateProperty);
  }
  return snapshot;
...
export function updateProperty(id, payload) {
  ...
  const next = properties.map((property) => {
    ...
    return {
      ...property,
      ...normalizeProperty(
        { listedAt: property.listedAt, ...payload },
        properties,
        property.id,
      ),
    };
