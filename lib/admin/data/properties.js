...
function normalizeProperty(payload, existing = [], currentId) {
  ...
  let paymentMilestones = normalizePaymentMilestones(payload.paymentMilestones);
  if (payload.paymentPlan) {
    const expanded = expandPaymentPlanToMilestones(payload.paymentPlan);
    if (!expanded.ok) {
      throw new Error(expanded.error);
    }
    paymentMilestones = expanded.milestones;
  }

  const bookingPercent =
    paymentMilestones
      .filter((item) => item.phase === "booking")
      .reduce((sum, item) => sum + item.percent, 0) || 10;
  const constructionCount = Math.max(
    1,
    paymentMilestones.filter((item) => item.phase === "construction").length,
  );
  ...
  return {
    ...
    paymentDownPercent: (() => {
      const value = toNumber(bookingPercent, 10);
      return value > 0 && value < 100 ? value : 10;
    })(),
    paymentInstallments: constructionCount,
