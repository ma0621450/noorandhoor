...
function FilterBar({ filterPrefix, propertyTypes }) {
  return (
    <div className="relative z-50 flex w-full max-w-[720px] flex-col gap-3 min-[850px]:w-auto min-[850px]:max-w-none min-[850px]:flex-row min-[850px]:items-center">
      <div className="w-full rounded-[10px] border border-[#eec876]/50 bg-white/20 backdrop-blur-md max-[849px]:grid max-[849px]:grid-cols-1 min-[600px]:max-[849px]:grid-cols-2 min-[850px]:flex min-[850px]:h-14 min-[850px]:w-auto min-[850px]:items-center min-[850px]:overflow-visible min-[850px]:rounded-[10px]">
        <DropdownGroup>
          <Dropdown
...
export default function PropertiesRentHero({
  title,
  description,
    ...
    <section className="relative isolate z-20 min-h-svh w-full overflow-x-clip overflow-y-visible">
      <HeroBackgroundCarousel overlayClassName="bg-black/60">
        <div className="absolute inset-0 flex h-full flex-col items-center justify-center gap-8 px-4 pt-24 pb-8 text-center sm:gap-[38px] sm:px-8 lg:px-20">
          ...
          <p className="max-w-[744px] text-sm leading-[1.4] font-light text-[#f5f5f5] drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] sm:text-base md:text-lg lg:text-2xl lg:leading-[33px]">
            {description}
          </p>

          <FilterBar filterPrefix={filterPrefix} propertyTypes={propertyTypes} />
