...
export default function BuyTestimonials() {
  return (
    <section className="section-full py-12 sm:py-14 lg:py-16">
      <div className="section-inner flex flex-col items-center">
        <div className="mb-8 flex max-w-[1118px] flex-col items-center gap-5 text-center sm:mb-10">
          ...
          <p className="text-sm font-medium leading-[26px] text-[#f5f5f5] sm:text-base">
            Hear directly from clients who trusted us with their biggest investment.
          </p>
        </div>

        <div className="grid w-full max-w-[1118px] grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3 xl:gap-12">
          {TESTIMONIALS.map((testimonial) => (
            <BuyTestimonialCard key={testimonial.id} testimonial={testimonial} />
          ))}
        </div>
      </div>
    </section>
